...
pthread_mutex_t  mutex_task_threads_activity, stdout_mutex;
...
void task(int thread_id) {
	sprintf(str_buffer, "Thread ID: %d wörking!", thread_id);
	cout(str_buffer);
	sleep(WORK_AMOUNT);
	sprintf(str_buffer, "Thread ID: %d done!", thread_id);
	cout(str_buffer);
}
...
void* periodic_thread(void* arg) {
	...
	sprintf(str_buffer, "Periodic thread ID: %d(period %d ms) reporting!", my_id, my_period_ms);
	cout(str_buffer);
...
void* non_periodic_thread(void* arg) {
	...
	sprintf(str_buffer, "Non-Periodic thread ID: %d reporting!", my_id);
	cout(str_buffer);
